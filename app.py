...
def upload_image():
    ...
    elif operation_selection == rgb_effect(decode_array_to_img):
        file_data = rgb_effect(decode_array_to_img)
    #write elif condition for Water, Invert and HDR image below
    elif operation_selection == water_effect(decode_array_to_img):
        file_data = water_effect(decode_array_to_img)
    elif operation_selection == invert_effect(decode_array_to_img):
        file_data = invert_effect(decode_array_to_img)
    elif operation_selection == hdr_effect(decode_array_to_img):
...
def hdr_effect(deocde_array_to_img):
    hdr_effect = cv2.detailEnhance(deocde_array_to_img, sigma_s=12, sigma_r=0.15)
